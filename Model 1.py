...
def detect_and_track_faces():
    ...
    while True:
        ...
        for face_id in list(tracking_faces.keys()):
            tracker, bbox, _, last_seen_time, consecutive_not_detected = tracking_faces[face_id]
            elapsed_time = time.time() - last_seen_time
            if elapsed_time > tracking_duration or consecutive_not_detected >= 10:
        ...
        for (x, y, w, h) in faces:
            ...
            tracked = False
            for fid, (tracker, bbox, _, last_seen_time, _) in tracking_faces.items():
            ...
            if not tracked:
                ...
                tracking_faces[next_face_id] = (tracker, bbox, (centroid_x, centroid_y), time.time(), 0)
                face_id = next_face_id
                next_face_id += 1
                tracker.init(frame, bbox)
            else:
                tracking_faces[face_id] = (tracker, bbox, (centroid_x, centroid_y), time.time(), 0)
